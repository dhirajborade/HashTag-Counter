...
public class FibonacciHeap {
	...
	public void insert(NodeFibHeap newNode) {
		...
		if (max == null) {
			...
			// Create a tree of single element and add it to the top level circular list and
	 ...
	 * 
	 * @param node
	 *            traversal of top-list starts from this node while updating max
	 */
	...
	/**
	 * Removes maximum node from the Fibonacci heap Updates new maximum and
	 * consolidates top circular list trees until no trees of equal rank are left
	 * Uses meld operation internally
	 * 
	 * @return removed node from Fibonacci heap
	 * @throws Exception
	 *             when trying to remove a node when heap is empty
	 */
	public NodeFibHeap remove() throws Exception {
		...
		if (removeNode.child != null) {
			// Case1: Maximum is the single element in the top level list; Case2: Top level
		...
		NodeFibHeap[] ranks = new NodeFibHeap[heapMaxDegree + 1]; // heapMaxDegree+1 will create array with highest
																	// index as heapMaxDegree
		NodeFibHeap temp = max, current; // current stores copy of the current node since the node links are getting
											// dynamically updated
		...
		do {
			...
			while (ranks[temp.degree] != null) {
				if (ranks[temp.degree] == temp) {
					System.out.print(temp.degree + " " + temp.key + " " + max.key);
					break;
				}
	...
	/**
	 * Melds two trees Removes the to be child tree from top-level circular list and
	 * adds it to the to be parent's child circular list, updating the sibling and
	 * parent pointers. Since meld happens only during remove node, to bypass
	 * redundant traversal of top-level root nodes we remove the root of the melded
	 * tree from its position and put it at "current" position as defined by
	 * "current" variable in remove method
	 * 
	 * @param node1
	 *            current node in top-level circular list traversal where meld is
	 *            called
	 * @param node2
	 *            node with equal degree as current node
	 * 
	...
	private NodeFibHeap meld(NodeFibHeap node1, NodeFibHeap node2) {
		...
		if (node1.data < node2.data || (node1.data.equals(node2.data) && max == node2)) { // Added "=" since assuming
																							// maximum element will
																							// already be in the ranks
																							// array
			node1 = node2;
			node2 = root;
		}
	...
	/**
	 * Increases the key of a node and updates it in Fibonacci Heap and max if
	 * necessary
	 * 
	 * @param node
	 * @param increase
	...
	/**
	 * It checks whether the increased node in increaseKey is larger than its parent
	 * node, if so it will remove the increased node from its parent and add to top
	 * level circular list. While doing so to maintain the flat structure of the
	 * heap it will also remove the parent whose cascadingCut flag is marked true or
	 * in other sense lost a child in the past
	 * 
	 * @param temp
	 */
	public void cascadingCut(NodeFibHeap temp) {
		...
		// If any method is calling cascadingCut it means calling method encountered a
		// node whose second child
		// has been removed
	...
	/**
	 * Since nodes from lower level will be removed during both increase key and
	 * cascading cuts modularized to separate function
	 * 
	 * @param temp
	 * 
	...
	private boolean addLowerLevelNodeToTopLevellist(NodeFibHeap temp) {
		...
		// single child of parent then set parent's child as null and add to top level
		// list
